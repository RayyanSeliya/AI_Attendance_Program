...
for cu_img in mylist:
    current_Img = cv2.imread(f'{path}/{cu_img}')
    if current_Img is not None:  # Check if the image was loaded successfully
        Images.append(current_Img)
        PersonName.append(os.path.splitext(cu_img)[0])
print(PersonName)
...
while True:
    ...
    faces = cv2.resize(frame, (0, 0), None, 0.25, 0.25)
    faces = cv2.cvtColor(faces, cv2.COLOR_BGR2RGB)  # Changed 'frame' to 'faces'
    ...
    cv2.imshow("Camera", frame)
    if cv2.waitKey(1) == 13:  # Wait for the Enter key
        break
